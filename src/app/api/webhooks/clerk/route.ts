...
import { createUserFromClerk, updateUserFromClerk, getUserByClerkId } from '@/lib/db/user';
import {
  ...
  try {
    ...
    if (eventType === 'user.updated' && eventData.id) {
      ...
      if (existingUser) {
        ...
        await createUserFromClerk(clerkUser);
        console.log(`User created in database (from update event): ${clerkUser.id}`);
      }
    }
    ...
    if (eventType === 'subscription.created') {
      ...
      if (!userId || !subscriptionId) {
        console.error('Missing user_id or subscription id in subscription.created event');
      ...
      const status = eventData.status ? statusMap[eventData.status.toLowerCase()] || 'ACTIVE' : 'ACTIVE';
    ...
    if (eventType === 'subscription.updated') {
      ...
      const existingSubscription = await getSubscriptionByClerkId(subscriptionId);
      ...
      if (existingSubscription) {
        ...
        const status = eventData.status ? statusMap[eventData.status.toLowerCase()] : undefined;

        await updateSubscriptionFromClerk(subscriptionId, {
          plan: eventData.plan?.name,
          status,
          currentPeriodStart: eventData.current_period_start ? new Date(eventData.current_period_start * 1000) : undefined,
          currentPeriodEnd: eventData.current_period_end ? new Date(eventData.current_period_end * 1000) : undefined,
    ...
    if (eventType === 'checkout.session.completed') {
      ...
      if (!userId || !sessionId) {
        console.error('Missing user_id or session id in checkout.session.completed event');
        return NextResponse.json({ received: true });
      }
