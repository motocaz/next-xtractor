  ...
  return (
    <section className="py-20">
      <div className="mb-8 text-center">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground leading-tight mb-6 text-balance">
          ...
          <span className="inline-flex items-center gap-4 ml-4">
            <Shield className="w-8 h-8 md:w-10 md:h-10 text-primary bg-primary/20 rounded-lg p-1.5" />
            We keep
          </span>
          <br className="hidden sm:block" />
          <span className="inline-block border-2 border-primary bg-primary/20 text-primary/80 px-4 py-2 rounded-full mx-2 text-2xl md:text-3xl lg:text-4xl font-bold">
      ...
      <div className="mb-16 text-center">
        <span className="inline-flex items-center gap-2 text-primary text-lg font-medium transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-4">
        {complianceItems.map((item, index) => (
          <div key={index} className="flex flex-col items-center text-center">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-primary flex items-center justify-center mb-4">
              <Image
                src={item.image}
                alt={item.alt}
                width={96}
