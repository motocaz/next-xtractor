...
export const addWatermark = async (
  ...
  if (options.type === 'text') {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      const textWidth = watermarkFont.widthOfTextAtSize(options.text, options.fontSize);
      ...
      const offsetX = textCenterX * Math.cos(angleRad) - textCenterY * Math.sin(angleRad);
      const offsetY = textCenterX * Math.sin(angleRad) + textCenterY * Math.cos(angleRad);
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      const angleRad = (options.angle * Math.PI) / 180;
      const offsetX = imgCenterX * Math.cos(angleRad) - imgCenterY * Math.sin(angleRad);
      const offsetY = imgCenterX * Math.sin(angleRad) + imgCenterY * Math.cos(angleRad);
