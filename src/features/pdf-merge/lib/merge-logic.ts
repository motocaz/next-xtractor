...
export const mergePDFsPageMode = async (
  ...
  for (const pageData of pageOrder) {
    ...
    if (pageIndex < 0 || pageIndex >= pdfInfo.pdfDoc.getPageCount()) {
      console.warn(
        `Invalid page index: ${pageIndex} for file: ${pdfInfo.fileName}`,
      );
      continue;
    }

    try {
      const [copiedPage] = await newPdfDoc.copyPages(pdfInfo.pdfDoc, [
        pageIndex,
      ]);
      newPdfDoc.addPage(copiedPage);
    } catch (error) {
      console.error(
        `Error copying page ${pageIndex} from ${pdfInfo.fileName}:`,
        error,
      );
      throw new Error(
        `Failed to copy page ${pageData.pageNumber} from ${pdfInfo.fileName}`,
      );
    }
  }
