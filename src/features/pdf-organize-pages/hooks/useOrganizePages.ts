...
import { processOrganizedPages, buildPageOrder } from '../lib/organize-pages-logic';
import { renderAllPagesAsThumbnails } from '../lib/thumbnail-renderer';
import { downloadFile } from '@/lib/pdf/file-utils';
import type { UseOrganizePagesReturn, PageThumbnailData, PageThumbnail } from '../types';
...
export const useOrganizePages = (): UseOrganizePagesReturn => {
  ...
  const processAndSave = useCallback(async () => {
    ...
    try {
      ...
      const blob = await processOrganizedPages(pdfDoc, pageIndices);
