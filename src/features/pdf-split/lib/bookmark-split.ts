...
const collectBookmarkPages = (
  ...
  for (const bookmark of bookmarks) {
    ...
    if (bookmark.children.length > 0) {
      collectBookmarkPages(bookmark.children, targetLevel, currentLevel + 1, pages);
    }
  }
  return pages;
};
...
export const splitByBookmarks = async (
  ...
  for (let i = 0; i < sortedSplitPages.length; i++) {
    ...
    const endPage =
      i < sortedSplitPages.length - 1 ? sortedSplitPages[i + 1] - 1 : totalPages - 1;
  ...
  const baseName = originalFileName?.replace(/\.pdf$/i, '') || 'split-by-bookmarks';
